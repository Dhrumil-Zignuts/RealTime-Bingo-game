...
io.on('connection', (socket) => {
  ...
  socket.on('create-room', async ({ username, room }) => {
    ...
    try {
      const findUser = await MyModel.findOne({
        room: room,
        username: username,
      });
      if (!findUser) {
...
const MyModel = mongoose.model(
  'Users',
  new mongoose.Schema({ username: String, room: String, socketID: String })
);
mongoose.connect(
  'mongodb+srv://dhrumil:ZYOX8PGbAmTBbUZm@cluster0.kndl1f7.mongodb.net/realTimeBingo'
);
